      ...
      for (let bi = 0; bi < bassMeasures.length; bi++) {
        ...
        try {
          ...
          const vn2 = bmNotes.map((el: any) => {
            if (el.type === 'rest') {
              return new StaveNote({ keys: ['d/3'], duration: durationToVex(el.duration, el.dotted, true), clef: 'bass' });
            }
            return new StaveNote({ keys: [`${el.pitch.toLowerCase()}/${el.octave}`], duration: noteToVexDuration(el), clef: 'bass' });
          });

          const voice2 = new Voice({ num_beats: timeSignature.numerator, beat_value: timeSignature.denominator });
          ...
          try {
            ...
            beams2.forEach((b: any) => { try { b.setContext(context).draw(); } catch { /* ignora */ } });
