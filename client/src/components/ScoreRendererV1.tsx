...
import type { ParsedElement, ParsedNote, ParsedRest } from '../lib/brailleMusic_v1';
...
export default function ScoreRenderer({ elements, width = 1000, height = 300, beatsPerMeasure = 4, onNoteClick, onMeasureClick, highlightedSourceIndex }: ScoreRendererProps) {
  ...
  const timeSignature = useMemo(() => {
    const timeSigEl = elements.find(el => (el as any).type === 'timesignature') as any;
    if (timeSigEl) {
  ...
  const keySignature = useMemo(() => {
    const keySigEl = elements.find(el => (el as any).type === 'keysignature') as ParsedKeySignature | undefined;
